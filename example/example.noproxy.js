 ...
 */
var api = RESTObject.create('/example/api', null, false);
var customers = api.route('portal/users/customers');
$(function() {
  reloadList();
  $('button.save').on('click', function(event) {
    ...
    customers.route(item.id).update(item).then(function() {
      reloadList();
    });
  });
...
function editCustomer(item) {
  ...
  customers.route(item.id).read().then(
    displayForm,
    function(xhr) {
...
function deleteCustomer(item) {
  if (confirm('Delete customer?')) {
    ...
    customers.route(item.id).delete().then(function() {
